...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
	...
	const handleLike = async () => { 
		...
		await runTransaction(firestore, async (transaction) => {
			const { problemDoc, userDoc, userRef, problemRef } = await returnUserDataProblemData(transaction);
				...
				<div className='px-5'>
					...
					<div className='w-full'>
						...
						<div className='text-white text-sm'>
							<div dangerouslySetInnerHTML={{__html : problem?.problemStatement as string}} />
						</div>
...
function useGetCurrentProblem(problemId: string) { 
	...
	const [problemDifficultyClass, setProblemDifficultyClass] = useState<string>("bg");
	useEffect(() => {
		const gurCurrentProblem = async () => {
			setLoading(true)
